...
class BaseElement(object):
    ...
    def find_element(self):
        waiter = ec.presence_of_element_located(
            (self.get_search_condition(), self.get_locator()))
        element = self.wait_for_check_by_condition(method_to_check=waiter,
                                                   message=" не был найден")
        return element

    @staticmethod
    def get_displayed_elements(condition, locator):
        element_size = len(
            Browser.get_browser().get_driver().find_elements(condition,
                                                             locator))
        result_elements = []
        try:
    ...
    def get_elements(self):
        return Browser.get_browser().get_driver().find_elements(
            self.__search_condition, self.__locator)
    ...
    def get_elements_count(self):
        elements_count = len(Browser.get_browser().get_driver().find_elements(
            self.__search_condition, self.__locator))
    ...
    def wait_for(self, condition, *args, **kwargs):
        ...
        return WebDriverWait(Browser.get_browser().get_driver(),
                             Waits.EXPLICITLY_WAIT_SEC,
                             ignored_exceptions=[
                                 StaleElementReferenceException]).until(func)
    ...
    def actions_click_with_key(self, key):
        actions = ActionChains(Browser.get_browser().get_driver())
        actions.key_down(value=key).click(self.find_element()).key_up(
            value=key).perform()
    ...
    def wait_for_clickable(self):
        waiter = ec.element_to_be_clickable(
            (self.get_search_condition(), self.get_locator()))
        return self.wait_for_check_by_condition(method_to_check=waiter,
                                                message=" не доступен для "
                                                        "щелчка")

    def wait_for_is_visible(self):
        self.wait_for_is_present()
        waiter = ec.visibility_of_element_located(
            (self.get_search_condition(), self.get_locator()))
    ...
    def wait_for_text(self, text, wait_time_sec=Waits.EXPLICITLY_WAIT_SEC):
        waiter = ec.text_to_be_present_in_element(
            (self.get_search_condition(), self.get_locator()), text)
        self.wait_for_check_by_condition(method_to_check=waiter,
                                         message=" не содержит " + text,
                                         wait_time_sec=wait_time_sec)

    def wait_for_visibility(self):
        waiter = ec.visibility_of(self.find_element())
        self.wait_for_check_by_condition(method_to_check=waiter,
                                         message=" не стал видимым")

    def wait_for_invisibility(self):
        waiter = ec.invisibility_of_element_located(
            (self.get_search_condition(), self.get_locator()))
        self.wait_for_check_by_condition(method_to_check=waiter,
                                         message=" остался видимым")
    ...
    def get_n_element_from_top(self, number_from_top):
        ...
        for element in elements:
            y_pos_element_dict[element.find_element().location[
                BaseElement.coordinate_y]] = element
    ...
    def get_element_number_from_top(self, element_index=1):
        ...
        for i in range(0, len(elements)):
            number_from_top_and_index_list[i] = elements[i].location[
                BaseElement.coordinate_y]
        indices_dict = OrderedDict(
            sorted(number_from_top_and_index_list.items(), key=lambda t: t[1]))
    ...
    def get_dict_of_elements_attribute_and_vertical_location(condition,
        ...
        events_info = {}
        for element in events_time_elements:
            events_info[element.location[
                BaseElement.coordinate_y]] = element.get_attribute(
                attribute_name)
        return events_info
